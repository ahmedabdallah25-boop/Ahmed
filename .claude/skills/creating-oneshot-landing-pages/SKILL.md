...
> Sections 1–4 are the upstream skill, verbatim from
> [nroadley/Creating-Oneshot-Hero-Landing-Pages](https://github.com/nroadley/Creating-Oneshot-Hero-Landing-Pages/blob/main/SKILL.md)
> (vendored 2026-08-17). **Section 5 is a local addendum** — it maps the two Antigravity-only
> steps onto the tools this repo actually has. Read Section 5 before running Steps 1–2.
...
## 5. Local addendum — running this in this repo

*Not upstream. This section exists because Steps 1 and 2 above assume Antigravity, and this
repo does not run there.*

**What is missing here**

- There is no built-in `generate_image` tool (Nano Banana / `gemini-3.1-flash-image`).
- There is no `GEMINI_API_KEY` in the session and `google-genai` is not installed, so the
  Section 3 script cannot run as written. Keep it — it is correct, and it works the moment a
  key exists. Just do not plan around it by default.

**Substitutions — Step 1, reference images**

Use an image tool that is actually connected, and save the result into `./assets/`:

- `mcp__Higgsfield__generate_image` — the general default.
- `mcp__ElevenLabs__creative_generate_image` — returns as soon as generation starts, so poll
  `creative_get_flow_run_status` with the returned `flow_id` until it completes.

Both return URLs, not bytes. Download to `./assets/hero_ref.jpg` etc. with `curl -L` before
using them; the frontend must reference local files.

**Substitutions — Step 2, hero and showcase video**

- `mcp__Higgsfield__generate_video` for image-to-video from the Step 1 reference. Ask for
  16:9 and the longest duration the chosen model supports.
- `mcp__ElevenLabs__creative_generate_video` as the alternative; same flow-polling rule.

Load the tool schema with `ToolSearch` before calling — do not guess parameter names.

Two things the upstream 10s assumption hides: these models often cap below 10s, and the
scrub engine in Section 4 divides scroll progress by `heroVideo.duration`, so **any**
duration scrubs correctly. A 5s clip over 400vh is fine. Do not stretch or pad the file to
hit 10s.

Download the produced `.mp4` into `./assets/hero.mp4`. The blob preloader in Section 4
fetches that exact path.

**Step 4, verification**

`python3 -m http.server 3000 --directory <dir>` works normally. For the visual inspection
pass, Chromium is pre-installed at `/opt/pw-browsers/chromium` with
`PLAYWRIGHT_BROWSERS_PATH` already set — never run `playwright install`. Scripted scroll
plus screenshots at ~0%, 35%, 70% and 100% of the hero section is enough to confirm the
phase typography is progress-locked and the video is seeking.

**Repo fit**

Nothing here touches either YouTube channel. Landing pages built with this skill are
standalone; put them in their own directory and keep them clear of `automation/`,
`video/`, `media/` and `heldbyfaith/`.
