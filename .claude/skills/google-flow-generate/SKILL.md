...
Setup (once, on a local machine — not a remote/web container):
`bash tools/google-flow/setup.sh`. Server installs to `~/.mcp/google-flow-mcp`.
...
```
bash tools/google-flow/ensure-flow-chrome.sh
```

Windows PowerShell:

```
powershell -NoProfile -ExecutionPolicy Bypass -File "$HOME/.mcp/google-flow-mcp/scripts/ensure-flow-chrome.ps1"
```
...
- `WRONG_GOOGLE_ACCOUNT` → stop; setup mismatch (check `expectedAccount` in
  `~/.mcp/google-flow-mcp/config/flow.config.json`).
...
Image fallback: Flow → Gemini API (`~/.mcp/google-flow-mcp/fallback/gemini_image_fallback.py`,
needs `GEMINI_API_KEY`) → any per-credit generator you use (this repo's session also has
Higgsfield `generate_image`). Video fallback: Flow → your per-credit generator directly.

---
Adapted from the skill shipped with
[gabrielgargiulodev/google-flow-mcp](https://github.com/gabrielgargiulodev/google-flow-mcp) (MIT).
