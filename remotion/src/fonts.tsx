/**
 * Self-hosted Anton + Inter.
 *
 * Fonts are baked into public/fonts and injected as a <style> tag instead of
 * being pulled from Google at render time: the headless browser does not trust
 * this environment's proxy CA, so any gstatic.com request fails the render.
 * Self-hosting also makes the render reproducible offline.
 */
import React from 'react';
import {continueRender, delayRender, staticFile} from 'remotion';

const RAW = `/* vietnamese */
@font-face {
  font-family: 'Anton';
  font-style: normal;
  font-weight: 400;
...
const CSS = RAW.replace(/FONTS\/([^)'"\s]+)/g, (_m, f) => staticFile('fonts/' + f));

export const ANTON = 'Anton';
export const INTER = 'Inter';

/** Blocks the render until both families are actually rasterisable. */
